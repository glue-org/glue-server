...
export function createApp(): Express {
    ...
    app.set("trust proxy", 1); // trust first proxy, this is needed bc the app is behind an nginx reverse proxy
    ...
    app.use(
        session({
            ...
            cookie: {
                ...
                sameSite:
                    process.env.NODE_ENV === "production" ? "none" : "lax", // this is required for the cookie to be sent
                secure: process.env.NODE_ENV === "production" ? true : false, // https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie#secure
            ...
            store: store.create({ mongoUrl: process.env.MONGO_URL! }), // this URL has username and password encoded
        })
    );
